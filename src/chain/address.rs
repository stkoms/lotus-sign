...
impl Address {
    ...
    pub fn from_string(s: &str) -> Result<Self> {
        ...
        let payload = match protocol {
            Protocol::ID => encode_u64_varint(s[2..].parse()?),
            Protocol::Secp256k1 | Protocol::Actor | Protocol::BLS => base32_decode(&s[2..])?,
        };
...
impl fmt::Display for Address {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self.protocol {
            Protocol::ID => {
                let id = decode_u64_varint(&self.payload).map_err(|_| fmt::Error)?;
                write!(f, "{}{}", prefix, id)
            }
...
fn decode_u64_varint(bytes: &[u8]) -> Result<u64> {
    let mut value = 0u64;
    let mut shift = 0u32;

    for (i, &byte) in bytes.iter().enumerate() {
        if i == 10 {
            return Err(anyhow!("invalid f0 payload"));
        }
    ...
    Err(anyhow!("invalid f0 payload"))
}

#[cfg(test)]
mod tests {
    use super::{decode_u64_varint, encode_u64_varint, Address, Protocol};

    #[test]
    fn parses_id_address() {
        let addr = Address::from_string("f01953985").unwrap();
        assert_eq!(addr.protocol, Protocol::ID);
        assert_eq!(decode_u64_varint(&addr.payload).unwrap(), 1_953_985);
        assert_eq!(addr.to_string(), "f01953985");
    }

    #[test]
    fn id_varint_roundtrip() {
        let payload = encode_u64_varint(1_953_985);
        assert_eq!(decode_u64_varint(&payload).unwrap(), 1_953_985);
    }
}
