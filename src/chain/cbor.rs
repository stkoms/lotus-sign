...
pub fn serialize_withdraw_balance_params(amount: &BigInt) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
    write_cbor_array_header(&mut buf, 1);
    write_bigint(&mut buf, amount);
    Ok(buf)
}

pub fn serialize_change_owner_params(new_owner: &Address) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
    write_cbor_array_header(&mut buf, 1);
    write_address(&mut buf, new_owner);
    Ok(buf)
}

pub fn serialize_change_worker_params(
    new_worker: &Address,
    new_control_addresses: &[Address],
) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
    write_cbor_array_header(&mut buf, 2);
    write_address(&mut buf, new_worker);
    write_cbor_array_header(&mut buf, new_control_addresses.len() as u64);
    for addr in new_control_addresses {
        write_address(&mut buf, addr);
    }
    Ok(buf)
}

pub fn serialize_market_withdraw_params(
    provider_or_client: &Address,
    amount: &BigInt,
) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
    write_cbor_array_header(&mut buf, 2);
    write_address(&mut buf, provider_or_client);
    write_bigint(&mut buf, amount);
    Ok(buf)
}
...
fn write_cbor_array_header(buf: &mut Vec<u8>, len: u64) {
    if len < 24 {
...
#[cfg(test)]
mod tests {
    use super::{serialize_change_owner_params, serialize_withdraw_balance_params};
    use crate::chain::{Address, BigInt};

    #[test]
    fn withdraw_params_are_cbor_array() {
        let amount = BigInt::parse_token_amount("0.1").unwrap();
        let bytes = serialize_withdraw_balance_params(&amount).unwrap();
        assert_eq!(bytes[0], 0x81);
    }

    #[test]
    fn change_owner_params_are_cbor_array() {
        let addr = Address::from_string("f01953985").unwrap();
        let bytes = serialize_change_owner_params(&addr).unwrap();
        assert_eq!(bytes[0], 0x81);
    }
}
